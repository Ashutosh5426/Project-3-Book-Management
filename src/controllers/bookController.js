...
const createBook = async function (req, res) {
  try {
    ...
    if (!isValid(userId))
      return res
        .status(400)
        .send({ status: false, message: "Please enter the user. ⚠️" });
...
let getbyBookId = async function (req, res) {
    ...
    //   return res
    //     .status(200)
    //     .send({ status: true, message: "Book Lists", data: bookData });
...
const updateBook = async function (req, res) {
  try {
    ...
    const { title, excerpt, releaseDate, ISBN } = data; //----------destructure
    ...
    if (isValid(ISBN) && !ISBN.match(isbnRegex))
      return res
        .status(400)
        .send({ status: false, message: "You entered a invalid ISBN. ⚠️" });
    if (ISBN) {
    ...
    if (!findbook) // is any error possible.
  ...
  try {
    ...
    if (data) {
      if (!data.isDeleted) {
        ...
        res
          .status(200)
          .send({ status: false, message: "data already deleted ⚠️" });
      }
    }
  } catch (error) {
    res.status(500).send({ status: false, error: error.message });
